...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4 no-print">
  <h1 class="bb-page-title">
    <i class="bi bi-card-checklist bb-text-orange" aria-hidden="true"></i> <?= e($job['job_number']) ?>
  ...
  <div class="d-flex gap-2 flex-wrap">
    ...
    <?php if (is_admin()): ?>
      <a class="btn btn-outline-secondary" href="<?= base_url('job_cards/form.php?id=' . $id) ?>"><i class="bi bi-pencil" aria-hidden="true"></i> Edit</a>
    <?php endif; ?>
    <a class="btn btn-outline-secondary" href="<?= base_url('job_cards/index.php') ?>"><i class="bi bi-arrow-left" aria-hidden="true"></i> Back</a>
  </div>
</div>
...
<div class="row g-3">
  <div class="col-lg-8">
    <div class="card mb-3">
      <div class="card-header"><h2 class="h6 mb-0">Job Card <?= e($job['job_number']) ?></h2></div>
      <div class="card-body">
        <div class="row">
          <div class="col-md-6">
            <dl class="row mb-0">
              ...
              <dt class="col-5">Plate</dt><dd class="col-7 bb-mono"><?= e($job['plate_number']) ?></dd>
    ...
    <div class="card mb-3">
      <div class="card-header"><h2 class="h6 mb-0">Problem Description</h2></div>
      <div class="card-body"><p class="mb-0 bb-prewrap"><?= e($job['problem_description']) ?></p></div>
    </div>

    <div class="card">
      <div class="card-header d-flex align-items-center gap-2">
        <h2 class="h6 mb-0">Service Notes</h2>
        <span class="badge text-bg-secondary"><?= $noteCount ?></span>
      </div>
      <div class="card-body">
        <?php if (!$notes): ?>
          <p class="bb-empty mb-0">No service notes yet. At least one note is required before this job can be completed.</p>
        <?php else: ?>
          <?php foreach ($notes as $i => $note): ?>
            ...
            <div>
              ...
              <p class="mb-0 mt-1 bb-prewrap"><?= e($note['note']) ?></p>
            </div>
          <?php endforeach; ?>
        <?php endif; ?>
        ...
        <?php if ($canAddNote): ?>
          <form class="mt-3 no-print" method="post" action="<?= base_url('job_cards/note.php') ?>">
            ...
            <button class="btn btn-bb mt-2" type="submit"><i class="bi bi-plus-lg" aria-hidden="true"></i> Add Note</button>
          </form>
        <?php else: ?>
          <div class="alert alert-light border small mt-3 mb-0 no-print"><i class="bi bi-info-circle" aria-hidden="true"></i> This job card is cancelled and can no longer be updated.</div>
        <?php endif; ?>
      </div>
    </div>
  </div>
  ...
  <div class="col-lg-4 no-print">
    <div class="card">
      <div class="card-header"><h2 class="h6 mb-0">Update Status</h2></div>
      <div class="card-body">
        ...
        <?php if ($allowedStatuses): ?>
          <form method="post" action="<?= base_url('job_cards/status.php') ?>">
            <?= csrf_field() ?>
            <input type="hidden" name="id" value="<?= $id ?>">
            <label class="visually-hidden" for="status">New status</label>
            <select class="form-select mb-2" id="status" name="status" required>
              <?php foreach ($allowedStatuses as $s): ?>
                <option value="<?= e($s) ?>"><?= e($s) ?></option>
              <?php endforeach; ?>
            </select>
            <button class="btn btn-bb w-100" type="submit"><i class="bi bi-arrow-repeat" aria-hidden="true"></i> Update Status</button>
          </form>
        <?php else: ?>
        ...
        <?php if ($job['status'] !== 'Completed' && $noteCount === 0): ?>
          <div class="alert alert-warning small mt-3 mb-0" role="alert">
            <i class="bi bi-exclamation-triangle" aria-hidden="true"></i> Add at least one service note before this job can be marked Completed.
          </div>
        <?php endif; ?>
      </div>
    </div>
